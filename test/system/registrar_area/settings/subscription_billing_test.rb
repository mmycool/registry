...
class RegistrarAreaSettingsSubscriptionBillingTest < ApplicationSystemTestCase
  ...
  def test_show_details_when_active
    @registrar.subscription_billing.update!(active: true, balance_threshold: 1, top_up_amount: 10)
    visit registrar_settings_root_url
  ...
  def test_update
    @registrar.subscription_billing.update!(balance_threshold: 1, top_up_amount: 10)

    visit registrar_settings_root_url
    click_link_or_button 'Edit'
    check 'Active'
    fill_in 'Balance threshold', with: '100'
    fill_in 'Top-up amount', with: '1000'
    click_on 'Update'
    registrars(:bestnames).subscription_billing.reload

    assert_equal Money.from_amount(100), @registrar.subscription_billing.balance_threshold
